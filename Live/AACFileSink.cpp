...
std::string AACFileSink::getAttribute(){
    std::stringstream ss;
    ss<<"a=rtpmap:"<<mPayloadType<<" mpeg4-generic/"<<mSampleRate<<"/"<<mChannels<<"\r\n";
    uint8_t index = 0;
    ...
    ss<<"a=fmtp:"<<mPayloadType<<" profile-level-id=1;";
    ss<<"mode=AAC-hbr;";
    ss<<"sizelength=13;indexlength=3;indexdeltalength=3;";
    ss<<"config="<<atoi(configStr);
...
void AACFileSink::sendFrame(MediaFrame* frame){
    ...
    memcpy(fullPayLoad.data()+4, frame->mFrameBuf.data(), aacSize);
    mRtpPacket.SetPayloadData(fullPayLoad.data(), 4+aacSize);
