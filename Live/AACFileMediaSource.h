...
public:
    static AACFileMediaSource* createNew(UsageEnvironment* env, std::string filePath);
    AACFileMediaSource(UsageEnvironment* env, std::string filePath);
    ~AACFileMediaSource();
...
private:
    ...
    bool setADTSHeader(uint8_t* in, int size);
    int getAACFrameLength();
private:
    struct ADTSHeader{
